;(function(){
	...
	module.exports=function(mysql,connection,query,cb){
		this.mysql=mysql;
		...
		this.fCount=0;
		this.error=false;
    this.reservedWords=['IS','INTERVAL','MICROSECOND','SECOND','MINUTE','HOUR','DAY','WEEK','MONTH','QUARTER','YEAR','SECOND_MICROSECOND','MINUTE_MICROSECOND','MINUTE_SECOND','HOUR_MICROSECOND','HOUR_SECOND','HOUR_MINUTE','DAY_MICROSECOND','DAY_SECOND','DAY_MINUTE','DAY_HOUR','YEAR_MONTH','MICROSECONDS','SECONDS','MINUTES','HOURS','DAYS','WEEKS','MONTHS','QUARTERS','YEARS'];
    this.mathOperators=['+','-','*','/','DIV','%','MOD'];
    this.comparisonOperators=['=','!=','>','>=','<','<=','IS','IS NOT','<=>','BETWEEN'];
		if(cb)this.cb=cb;
		return this;
	}

	module.exports.prototype.validField=function(value){
		return value.match(/[^a-zA-Z0-9_.]/)==null;
	}
	module.exports.prototype.validNumeric=function(value){
		var m=value.match(/^[\d]+(\.[\d]+){0,1}$/g);
		return (typeof m=='object'&&m!==null&&m.indexOf(value)!==-1)?true:false;
	}
	module.exports.prototype.validTable=function(value){
		return value.match(/[^a-zA-Z0-9_]/)==null;
	}
	module.exports.prototype.validFunction=function(value){
		return value.match(/[^a-zA-Z0-9_]/)==null;
	}
	module.exports.prototype.validComparisonOperator=function(value){
		 return(this.comparisonOperators.indexOf(value.toUpperCase())!==-1)?true:false;
	}
	module.exports.prototype.validMathOperator=function(value){
		 return(this.mathOperators.indexOf(value.toUpperCase())!==-1)?true:false;
	}
	module.exports.prototype.validReservedWord=function(value){
		 return(this.reservedWords.indexOf(value.toUpperCase())!==-1)?true:false;
	}

	module.exports.prototype.option=function(name,value){
		if(value)
			return this.q[name];
		this.q[name]=value;
		return this;
	}
	module.exports.prototype.cb=function(cb){
		if(cb){
			this.cb=cb;
			return this;
		}
	...
	module.exports.prototype.execute=function(cb){
		var self=this;
		if(cb)this.cb=cb;
		this.buildTables();
		this.buildSelect();
		...
		this.connection.query(this.query,this.values,function(err,results){
			if(err){
				console.log('| Query Error '+err);
				self.cb(false);
			}else{
				self.connection.query('SELECT FOUND_ROWS() AS found',this.values,function(err,found){
					if(err){
						console.log('| Query Error '+err);
						self.cb(false);
					}else{
						if(results.length){
							fields=_.map(results[0],function(value,key){return key});
						}else{
							fields={};
						}
						self.cb({results:results,total:found[0].found,fields:fields});
					}
				});
			}
		});
	};
	module.exports.prototype.buildTables=function(){
	...
	module.exports.prototype.buildSelect=function(){
	...
	module.exports.prototype.buildJoins=function(){
		this.query+=' FROM '+this.mysql.escapeId(this.q.table);
		if(typeof this.q.join=='object'&&this.q.join.length){
			for(var i in this.q.join){
				var t=this.q.join[i].table;
				var index=t.toUpperCase().indexOf(' AS `');
				...
				this.query+=' '+(this.q.join[i].type?this.q.join[i].type.toUpperCase():'JOIN')+' '+this.mysql.escapeId(t)+(as?(' AS '+this.mysql.escapeId(as)):'')+' ON '+this.conditions(this.q.join[i].on);
			}
		}
	}
	module.exports.prototype.buildWhere=function(){
		if(typeof this.q.where!='object'||!this.q.where.length)return;
		this.query+=' WHERE '+this.conditions(this.q.where);
	}
	module.exports.prototype.buildGroupBy=function(){
	...
	module.exports.prototype.buildHaving=function(){
		if(typeof this.q.having!='object'||!this.q.where.having)return;
		this.query+=' HAVING '+this.conditions(this.q.having);
	}
	module.exports.prototype.buildOrderBy=function(){
		if(typeof this.q.orderBy!='object'||!this.q.orderBy)return;
		var build='';
	...
	module.exports.prototype.buildLimitOffset=function(){
		if(typeof this.q.limit=='number'){
			this.query+=' LIMIT '+this.mysql.escape(this.q.limit);
			if(typeof this.q.offset=='number')this.query+=' OFFSET '+this.mysql.escape(this.q.offset);
		}
	}
	module.exports.prototype.conditions=function(cond){
	...
	module.exports.prototype.parseField=function(field){
    var as=false;
    if(field==null)return {'type':'null',place:'NULL','as':this.enterAs(as,'null')};
    ...
    if(field[0]=='`'&&field[field.length-1]=='`'&&this.validField(field.substr(1,field.length-2))){
      ...
      return {'type':'field','table':this.q.table,'place':this.q.table+'.'+field,'as':this.enterAs(as,this.q.table+'.'+field)};
    }else if(this.validField(field)&&field.split('.').length==2){
      field=field.split('.');
      return {'type':'field','table':field[0],'place':field[0]+'.'+field[1],'as':this.enterAs(as,field[0]+'.'+field[1])};
    }else if(field.toUpperCase()=='NULL'){
      return {'type':'null',place:'NULL','as':this.enterAs(as,'null',true)};
    }else if(this.validReservedWord(field)){
      return {'type':'reserved','place':field,'as':this.enterAs(as,field,true,false)};
    }else if(this.validMathOperator(field)){
      return {'type':'math_operator','place':field,'as':this.enterAs(as,'math_operator',true)};
    }else if(this.validNumeric(field)){ //numeric
    	this.values.push(+field)
      return {'type':'numeric','place':'?','as':this.enterAs(as,'numeric',true)};
    }else if(field[0]=='('&&field[field.length-1]==')'){
      var walk=this.parseWalk(field.substr(1,field.length-2).trim());
      return {'type':'bracket','place':'( '+walk+' )','as':this.enterAs(as,'bracket',true)};
      ...
      if(this.validFunction(fn))return{'type':'function','function':fn,'place':fn.toUpperCase()+'( '+walk+' )','as':this.enterAs(as,fn,true)};
    }
    if((field[0]=="'"&&field[field.length-1]=="'")||(field[0]=='"'&&field[field.length-1]=='"'))field=field.substr(1,field.length-2);
    this.values.push(field);
    return{'type':'string','place':'?','as':this.enterAs(as,'string',true)};
  }
	module.exports.prototype.parseWalk=function(w){
		var brackets=0;
		var quote=false;
		var index=0;
		var build='';
	...
	module.exports.prototype.enterAs=function(as,def,increment,noQuotes){
		this.fCount++;
